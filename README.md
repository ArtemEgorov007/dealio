...
- **Нашли и закрыли реальный риск дублирования выдачи**, не дожидаясь жалобы: при прямом заходе на `/badges` (закладка, перезапуск телефона) экран раньше зависал без сотрудника и цеха в контексте — поправили роутинг (мидлварь не переживал конечный слэш от GitHub Pages).
...
4. **Получение бирки** — копирование в буфер, запись в журнал, пометка бирки выданной; кнопки прилипают к низу экрана на любом размере
