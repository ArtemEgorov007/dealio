...
2. Единая обработка ошибок: сейчас `error instanceof Error ? ... : '...'`
   повторяется в ~19 местах.
3. Автоматический повтор запроса к складскому GAS: баланс периодически
   отвечает ошибкой связи сразу после записи, сейчас лечится только ручной
   кнопкой «Повторить».
4. Проверить автопродление SSL (Let's Encrypt, ~90 дней).
