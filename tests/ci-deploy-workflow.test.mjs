...
test('deploy workflow targets hosting environments instead of GitHub Pages', () => {
    ...
    assert.match(workflow, /secrets\.NUXT_PUBLIC_ERP_GAS_URL \|\| secrets\.NUXT_PUBLIC_CRM_GAS_URL/)
    assert.doesNotMatch(workflow, /deploy-pages|upload-pages-artifact|configure-pages|FTP-Deploy-Action|rsync[^\n]*--delete|mirror[^\n]*--delete|sftp[^\n]*\brm\b/)
})
