...
function doPost(e) {
    try {
        ...
        if (payload.action === 'stagingAppendTestNotificationRows') {
            assertApprovalsToken_(payload.token || '')
            return jsonResponse_(stagingAppendTestNotificationRows())
        }
        if (payload.action === 'stagingAppendTestNotificationRowForManager') {
            assertApprovalsToken_(payload.token || '')
            return jsonResponse_(stagingAppendTestNotificationRowForManager(payload.managerFio || ''))
        }
...
function stagingAppendTestNotificationRowForManager(managerFio) {
    const sheet = approvalsSheet_()
    const context = approvalRows_(sheet)
    const header = context.header
    const invoiceColumn = header[normalizeCell_('Счет')]
    const managerColumn = header[normalizeCell_('Согласование руководителя')]
    if (invoiceColumn === undefined || managerColumn === undefined) throw new Error('approvals_column_missing')
    ...
    const suffix = Utilities.formatDate(new Date(), 'Europe/Moscow', 'yyyyMMdd-HHmmss')
    const invoice = `ERP-TEST-EGOROV-${suffix}`
    const row = template.values.slice()
    row[invoiceColumn] = invoice
    row[managerColumn] = String(managerFio || '').trim()
    ...
    sheet.appendRow(row)
    return {ok: true, invoice: invoice, rowNumber: sheet.getLastRow(), managerFio: row[managerColumn]}
}
